...
_ = try await run(.name("echo"), arguments: [String(decoding: JSONEncoder().encode(Array(selectedPackages)), as: UTF8.self)], output: .fileDescriptor(.open(githubOutputPath, .writeOnly), closeAfterSpawningProcess: true))
